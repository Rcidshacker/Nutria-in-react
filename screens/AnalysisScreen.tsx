...
const mockTodayProgress: TodayProgressData = { totalCalories: 1850, calorieGoal: 2200, protein: { grams: 120, percentage: 80, goalGrams: 150 }, carbs: { grams: 200, percentage: 73, goalGrams: 275 }, fat: { grams: 60, percentage: 86, goalGrams: 70 }, };
const mockWeeklyProgress: WeeklyProgressData = { averageDailyCalories: 2100, averageProtein: { grams: 135, percentage: 90 }, averageCarbs: { grams: 250, percentage: 91 }, averageFat: { grams: 65, percentage: 93 }, };
const mockWeightLog: WeightPoint[] = [ { date: 'Jul 1', weight: 70.5 }, { date: 'Jul 8', weight: 70.1 }, { date: 'Jul 15', weight: 69.8 }, { date: 'Jul 22', weight: 69.5 }, { date: 'Jul 29', weight: 69.2 }, ];
const currentWeight = mockWeightLog.length > 0 ? mockWeightLog[mockWeightLog.length - 1].weight : 0;
const weightGoal = 65;
const mockDailyCalories: DailyCalorieLog[] = [ { day: 'Mon', calories: 2300 }, { day: 'Tue', calories: 2150 }, { day: 'Wed', calories: 2200 }, { day: 'Thu', calories: 1900 }, { day: 'Fri', calories: 2400 }, { day: 'Sat', calories: 2500 }, { day: 'Sun', calories: 1850 },  ];
...
const ChartSkeleton: React.FC = () => (
  <div className="w-full h-[150px] bg-gray-200 dark:bg-gray-700 rounded-md animate-pulse"></div>
);
...
export const AnalysisScreen: React.FC<AnalysisScreenProps> = ({ navigateTo }) => {
  ...
  const chartLineColor = theme === 'dark' ? '#A3E635' : '#84CC16';
  ...
  const WeightChart: React.FC<{ data: WeightPoint[] }> = ({ data }) => {
    ...
    const points = data.map((point, i) => { const x = (data.length > 1 ? (width - 2 * padding) / (data.length - 1) * i : (width - 2 * padding) / 2) + padding; const y = height - padding - ((point.weight - minWeight) / weightRange * (height - 2 * padding)); return `${x},${y}`; }).join(' ');
    const XAxisLabels = () => ( <> {data.map((point, i) => { const x = (data.length > 1 ? (width - 2 * padding) / (data.length - 1) * i : (width - 2 * padding) / 2) + padding; return <text key={`x-label-${i}`} x={x} y={height - padding + 15} fontSize="10" fill={chartTextColor} textAnchor="middle">{point.date.substring(0,3)}</text> })} </> );
    const YAxisLabels = () => { const labels = [minWeight, minWeight + weightRange/2, maxWeight].map(w => Math.round(w*10)/10); return ( <> {labels.map((label, i) => { const y = height - padding - ((label - minWeight) / weightRange * (height - 2 * padding)); return <text key={`y-label-${i}`} x={padding - 5} y={y} fontSize="10" fill={chartTextColor} textAnchor="end" dominantBaseline="middle">{label}kg</text> })} </> ); };
    return ( <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto"> <line x1={padding} y1={padding} x2={padding} y2={height - padding} stroke={chartTextColor} strokeWidth="0.5" /> <line x1={padding} y1={height - padding} x2={width - padding} y2={height - padding} stroke={chartTextColor} strokeWidth="0.5" /> <YAxisLabels /> <XAxisLabels /> <polyline points={points} fill="none" stroke={chartLineColor} strokeWidth="2" /> {data.map((point, i) => { const x = (data.length > 1 ? (width - 2 * padding) / (data.length - 1) * i : (width - 2 * padding) / 2) + padding; const y = height - padding - ((point.weight - minWeight) / weightRange * (height - 2 * padding)); return <circle key={`point-${i}`} cx={x} cy={y} r="3" fill={chartLineColor} />; })} </svg> );
  };
  ...
  const CalorieBarChart: React.FC<{ data: DailyCalorieLog[] }> = ({ data }) => {
    ...
    const XAxisLabels = () => ( <> {data.map((day, i) => { const x = padding + i * (barWidth + barPadding) + barWidth / 2; return <text key={`x-label-bar-${i}`} x={x} y={height - padding + 15} fontSize="10" fill={chartTextColor} textAnchor="middle">{day.day}</text>; })} </> );
    const YAxisLabels = () => { const labels = [0, Math.round(maxCal/2), Math.round(maxCal)]; return ( <> {labels.map((label,i) => { const y = height - padding - (label / maxCal * (height - 2 * padding)); return <text key={`y-label-bar-${i}`} x={padding -5} y={y} fontSize="10" fill={chartTextColor} textAnchor="end" dominantBaseline="middle">{label}</text> })} </> ); };
    return ( <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto"> <line x1={padding} y1={padding} x2={padding} y2={height - padding} stroke={chartTextColor} strokeWidth="0.5" /> <line x1={padding} y1={height - padding} x2={width - padding} y2={height - padding} stroke={chartTextColor} strokeWidth="0.5" /> <YAxisLabels /> <XAxisLabels /> {data.map((day, i) => { const barHeight = (day.calories / maxCal) * (height - 2 * padding); const x = padding + i * (barWidth + barPadding); const y = height - padding - barHeight; return ( <rect key={day.day} x={x} y={y} width={barWidth} height={barHeight > 0 ? barHeight : 0} fill={chartBarColor} rx="2" /> ); })} </svg> );
  };
  ...
  return (
    <div className="p-4 sm:p-6 max-w-screen-lg mx-auto pb-20 space-y-8">
      ...
      <motion.section 
        variants={cardVariants} initial="hidden" animate="visible" transition={{ duration: 0.4, delay: 0.1 }}
        className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg transform transition-all duration-300 ease-in-out hover:shadow-xl hover:-translate-y-1"
      >
      ...
      <motion.section 
        variants={cardVariants} initial="hidden" animate="visible" transition={{ duration: 0.4, delay: 0.2 }}
        className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg transform transition-all duration-300 ease-in-out hover:shadow-xl hover:-translate-y-1"
      >
      ...
      <motion.section 
        variants={cardVariants} initial="hidden" animate="visible" transition={{ duration: 0.4, delay: 0.3 }}
        className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg transform transition-all duration-300 ease-in-out hover:shadow-xl hover:-translate-y-1"
      >
      ...
      <motion.section 
        variants={cardVariants} initial="hidden" animate="visible" transition={{ duration: 0.4, delay: 0.4 }}
        className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg transform transition-all duration-300 ease-in-out hover:shadow-xl hover:-translate-y-1"
      >
