...
// FIX: Removed unused 'AppRoute' from the import
...
export const ScanScreen: React.FC<ScanScreenProps> = () => {
  ...
  const simulateAnalysis = async () => {
    ...
    const mockResult: OCVAnalysisResult = {
      ...
      portionSize: "Approx. 300g",
      nutrients: { protein: "12g", fat: "10g", carbs: "45g", calories: "320 kcal" },
      ingredients: "Paneer, cream, tomato, spices, butter, onion",
      confidence: 0.89,
    };
  ...
  const getSmartSuggestions = (): OCVSmartSuggestion[] => {
    ...
    const proteinValue = parseInt(analysisData.nutrients.protein);
    if (proteinValue > 10) suggestions.push({ type: 'positive', text: 'High Protein', icon: '⚡' });
    const caloriesValue = parseInt(analysisData.nutrients.calories);
    if (caloriesValue > 300) suggestions.push({ type: 'warning', text: 'High Calorie', icon: '⚠️' });
    const fatValue = parseInt(analysisData.nutrients.fat);
    if (fatValue > 8) suggestions.push({ type: 'negative', text: 'High Fat', icon: '🔴' });
    return suggestions;
  };

  const handleEditStart = (cardType: string) => {
  ...
  const renderAnalysisCard = (
      ...
      label: string, 
      value: string, 
      editKey: keyof OCVAnalysisResult, 
      onEdit: () => void
    ) => (
    <div className="bg-white dark:bg-cocoa-800 rounded-xl p-4 shadow-md transform transition-shadow hover:shadow-lg">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {editingCard === editKey ? (
              <input
                type="text"
                value={editableData?.[editKey] as string || ''}
  ...
  return (
    <div className="min-h-screen bg-peach-100 dark:bg-cocoa-900 pb-20">
      <div className="max-w-md mx-auto p-4 sm:p-6 space-y-6">
        ...
        {!imagePreview ? (
          <div className="bg-white dark:bg-cocoa-800 rounded-xl border-2 border-dashed border-clay-300 dark:border-cocoa-600 p-8 text-center">
            <div className={`mx-auto w-16 h-16 bg-${PRIMARY_COLOR_CLASS}-100 dark:bg-cocoa-700 rounded-full flex items-center justify-center`}>
              <Camera className={`w-8 h-8 text-${PRIMARY_COLOR_CLASS}-600 dark:text-${PRIMARY_COLOR_CLASS}-400`} />
            </div>
            <p className="text-lg font-medium text-cocoa-800 dark:text-peach-100 mt-4 mb-2">
              Snap or upload your meal photo
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center mt-6">
              <Button onClick={handleCameraCapture} fullWidth className="flex justify-center items-center">
                <Camera className="w-5 h-5 mr-2" /> Camera
              </Button>
              <Button onClick={handleFileUpload} variant="outline" fullWidth className="flex justify-center items-center">
          ...
          <motion.div 
            ...
            className="relative bg-white dark:bg-cocoa-800 rounded-xl overflow-hidden shadow-lg"
          >
            <img src={imagePreview} alt="Food preview" className="w-full h-64 object-cover" />
            <button
              onClick={handleReset}
              className="absolute top-3 right-3 w-8 h-8 bg-black bg-opacity-40 rounded-full flex items-center justify-center text-white hover:bg-opacity-60 transition-colors z-10"
            >
              <X className="w-4 h-4" />
            </button>
            <AnimatePresence>
            {isAnalyzing && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute inset-0 bg-white/80 dark:bg-cocoa-900/80 backdrop-blur-sm flex items-center justify-center"
              >
        ...
        <AnimatePresence>
            ...
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
                <motion.div
                    ...
                    className="bg-white dark:bg-cocoa-800 rounded-2xl p-6 text-center max-w-sm w-full"
                >
                    ...
                    <h3 className="text-lg font-semibold text-cocoa-800 dark:text-peach-100 mb-2">
                        Meal Saved!
                    </h3>
                    <p className="text-cocoa-600 dark:text-clay-300">
