...
const initialWeightLog: WeightEntry[] = [ { id: 'w1', date: '2024-07-01', value: 75, notes: 'Starting weight' }, { id: 'w2', date: '2024-07-15', value: 74, notes: 'Felt good' }, { id: 'w3', date: '2024-07-28', value: 73.5, notes: 'After consistent workouts' }, ];
const initialPhotos: ProgressPhoto[] = [ { id: 'p1', date: '2024-07-01', imageUrl: 'https://picsum.photos/seed/progress1/300/300', notes: 'Day 1' }, { id: 'p2', date: '2024-07-20', imageUrl: 'https://picsum.photos/seed/progress2/300/300', notes: 'Mid-month check-in' }, ];
const initialAchievements: Achievement[] = [ { id: 'a1', title: '7-Day Tracking Streak', iconName: 'trophy', dateAchieved: '2024-07-08', description: 'Logged weight for 7 consecutive days!', color: ACCENT_COLOR_CLASS }, { id: 'a2', title: 'First 2kg Lost!', iconName: 'trophy', dateAchieved: '2024-07-28', description: 'Congratulations on losing your first 2 kilograms.', color: PRIMARY_COLOR_CLASS }, ];
const otherMetrics: { id: MeasurementEntry['metricName']; name: string; icon: React.FC<{className?: string}>; unit: string; latestValue?: number }[] = [ { id: 'bodyFat', name: 'Body Fat %', icon: Ruler, unit: '%', latestValue: 22 }, { id: 'waist', name: 'Waist', icon: Ruler, unit: 'cm', latestValue: 85 }, { id: 'hips', name: 'Hips', icon: Ruler, unit: 'cm' }, ];
...
export const ProgressScreen: React.FC = () => {
  ...
  const [newWeightEntry, setNewWeightEntry] = useState<{ value: string; date: string; notes: string }>({ value: '', date: new Date().toISOString().split('T')[0], notes: '' });
  ...
  const handlePhotoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        const newPhoto: ProgressPhoto = { id: `p${Date.now()}`, date: new Date().toISOString().split('T')[0], imageUrl: reader.result as string, notes: `Uploaded on ${new Date().toLocaleDateString()}` };
  ...
  return (
    <div className="p-4 sm:p-6 max-w-screen-lg mx-auto pb-20 space-y-8">
      ...
      <motion.section variants={cardVariants} initial="hidden" animate="visible" transition={{ duration: 0.4, delay: 0.1 }}>
        <div className="flex justify-center space-x-2 mb-4">
          {(['1M', '3M', '6M', 'All'] as const).map(filter => (
            <Button key={filter} variant={dateFilter === filter ? 'primary' : 'outline'} size="sm" onClick={() => setDateFilter(filter)}>
              {filter}
            </Button>
          ))}
        </div>
      ...
      <motion.section 
        ...
        className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow-lg transform transition-shadow hover:shadow-xl"
      >
      ...
      <motion.div variants={cardVariants} initial="hidden" animate="visible" transition={{ duration: 0.4, delay: 0.3 }} className="text-center">
      ...
      <motion.section variants={cardVariants} initial="hidden" animate="visible" transition={{ duration: 0.4, delay: 0.4 }}>
      ...
      <motion.section variants={cardVariants} initial="hidden" animate="visible" transition={{ duration: 0.4, delay: 0.5 }}>
      ...
      <motion.section variants={cardVariants} initial="hidden" animate="visible" transition={{ duration: 0.4, delay: 0.6 }}>
      ...
      {showWeightModal && (
        ...
        >
          <div 
            className={`bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-md transform transition-all duration-300 ease-in-out ${showWeightModal ? 'opacity-100 scale-100' : 'opacity-0 scale-95 translate-y-4 pointer-events-none'}`}
          ...
          >
            ...
            <Input
              label="Current Weight (kg)" id="newWeightValue" type="number" value={newWeightEntry.value}
              onChange={(e) => setNewWeightEntry(prev => ({ ...prev, value: e.target.value }))}
              placeholder="e.g. 70.5" icon={<PlusCircle className="text-gray-400" />} autoFocus
            />
            <Input
              label="Date" id="newWeightDate" type="date" value={newWeightEntry.date}
              onChange={(e) => setNewWeightEntry(prev => ({ ...prev, date: e.target.value }))}
              icon={<CalendarDays className="text-gray-400" />}
            />
            <Input
              label="Notes (Optional)" id="newWeightNotes" type="text" value={newWeightEntry.notes}
