...
export const Select: React.FC<SelectProps> = ({ label, id, options, error, containerClassName = '', className, ...props }) => {
  return (
    <div className={`mb-4 ${containerClassName}`}>
      ...
      <select
        id={id}
        className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-melon-500 focus:border-melon-500 dark:bg-cocoa-700 dark:text-peach-100 transition-colors duration-200 ease-in-out ${className} ${error ? 'border-red-500' : 'border-clay-300 dark:border-cocoa-600'}`}
        {...props}
      >
